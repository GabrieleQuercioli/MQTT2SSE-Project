...
import com.example.MQTT2SSE_BackEnd.StatusMessage;
import com.example.MQTT2SSE_BackEnd.UserEntity;
import com.example.MQTT2SSE_BackEnd.repository.UserRepository;
...
@Service
@Primary
public class SseService implements UserService {

    private final UserRepository repository;

    public SseService(UserRepository repository) {
    ...
    public void addEmitter(String userID, SseEmitter emitter, String topic) {
        emitters.put(new Pair<>(userID, emitter), topic);
    }
    ...
    public void deleteUser(Long id) {
        System.out.println("non da impl");
    }

    @Override
    public void deleteUsers(String userID) {
        List<UserEntity> toDelete = findByUserName(userID);
        System.out.println("to delete: " + toDelete);
        for (UserEntity userToDelete : toDelete){
            repository.delete(userToDelete);
        }
    }

    @Override
    public void deleteUser(String userID, String topic) {
        List<UserEntity> toDelete = findByUserNameAndTopic(userID,topic);
        System.out.println("to delete: " + toDelete);
        for (UserEntity userToDelete : toDelete){
            repository.delete(userToDelete);
        }
   ...
   public static void dispatchEventsToClients(@RequestParam String topic, @RequestParam String payload) throws MessageHandlingException {
    ...
    @Override
    public List<UserEntity> findByUserNameAndTopic(String userName, String topic){
        return repository.findByUserNameAndTopic(userName,topic);
    }
